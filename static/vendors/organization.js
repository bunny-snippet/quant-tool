  ...
  function renderAccess() {
    ...
    function policyRows(unit) {
      ...
      return rows.map(({ row, sourceUnit }) => {
        const direct = directIds.has(Number(row.client)); const source = direct ? 'Direct policy' : `Inherited from ${sourceUnit.name}`;
        const editButton = direct ? actionButton('client-access', row.id, canManageUnitClients) : (canManageUnitClients ? `<button class="unit-action" type="button" data-override-client-access="${unit.id}:${row.client}" data-min-cpi="${escapeHtml(row.min_cpi ?? '')}" data-max-cpi="${escapeHtml(row.max_cpi ?? '')}" data-policy-active="${row.is_active ? 'true' : 'false'}">Override</button>` : '');
