...
(() => {
  ...
  const entryLinkControl = (survey) => survey.start_link
    ? `<button class="copy-link" data-copy-link="${escapeHtml(survey.start_link)}">Copy link</button>`
    : '<button class="copy-link" type="button" title="This survey is not live" disabled>Unavailable</button>';
  ...
  function cardTemplate(survey) {
    ...
    const bottom = `${projectColumns.has('modified') ? `<div class="source-timestamp"><small>Provider modified</small>${sourceTimestamp(survey.source_modified_display, survey.source_modified_at)}<small>Fetched ${escapeHtml(formatDate(survey.last_seen_at))}</small></div>` : ''}${projectColumns.has('entry_link') ? entryLinkControl(survey) : ''}`;
